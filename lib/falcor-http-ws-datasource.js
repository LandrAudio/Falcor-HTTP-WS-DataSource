...
import FalcorHttpDataSource from '@landr/falcor-http-datasource';
...
class FalcorHttpPullWebSocketPushDataSource extends FalcorHttpDataSource {
  ...
  _needRetryOnErrors(method, args, node) {
    ...
    if ($type === 'error') {
      ...
      return Promise.all(needRetries).then(retries => retries.reduce((r, needRetry) => r || needRetry, false));
    }
  ...
  _monitorRequest(method, args, res) {
    ...
    let observable = Observable.create((observer) => {
      let needRetryResolve;
      let validateNeedRetry = new Promise((resolve) => {
      ...
      res.subscribe(
        ...
        () => {
          validateNeedRetry.then(needRetry => {
            if (!needRetry) {
              this._finishedRequest();
              observer.onCompleted();
            }
